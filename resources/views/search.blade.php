<x-app-layout>
    ...
    <table class="table">
        ...
        <tbody>
            ...
            <tr>
                <th scope="row">{{$fichier->tmp_nom_fichier}}</th>
                <td>{{$fichier->nom_matiere}}</td> 
                <td>{{$fichier->semestres}}</td> 
                <td>{{$fichier->tmp_size_fichier}}</td> 
                <td><a href=" {{url('/download', $fichier->tmp_url_fichier)}} " class="btn btn-success">Télécharger</a></td>
            </tr>
            @endforeach
