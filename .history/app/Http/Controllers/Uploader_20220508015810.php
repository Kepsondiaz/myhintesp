    ...
    public function index(Request $request)
    {
               ...
               if (request()->hasFile('fichier')) {
                    ...
                    if(($extenFileUpload == 'pdf' || $extenFileUpload == 'PDF' || $extenFileUpload == 'word' || $extenFileUpload == 'WORD') && $sizeFileUpload <= 300000)
                    {
                         ...
                         return session()->flash('message_succes', 'fichier uploader avec succé !');
                         // return view('dashboard');    
                    }
                    else
                    {
                         session()->flash('message_danger', 'Ce type d\'extension n\'est pas autorisée ou la taille de ce fichier n'es ! '); // affichage de message d'erreur
